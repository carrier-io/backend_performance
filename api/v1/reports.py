...
def write_test_run_logs_to_minio_bucket(report: Report):
    ...
    if loki_settings_url:
        ...
        llf = LokiLogFetcher(url=loki_url)
        try:
            llf.fetch_logs(query=logs_query)
            llf.to_file(file_output, enc=enc)
            minio_client = MinioClient.from_project_id(report.project_id)
            ...
            minio_client.upload_file(bucket_name, file_output, f"{build_id}.log")
        except:
